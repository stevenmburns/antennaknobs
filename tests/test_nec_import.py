...
def test_skipped_note_renders_ignored_cards_and_ground():
    ...
    # Descriptions come from _IGNORED_CARDS (the single source of truth), and
    # mnemonic case survives. GN stays in `ignored` but is not called "not
    # applied": the app seeds its ground from GE/GN (AK#1432), and saying
    # otherwise contradicted the ground panel (AC6LA, 2026-09-13).
    assert note == (
        "Deck cards not applied: LD (loading) — the app's own settings are used "
        "instead."
    )


def test_gn_minus_one_is_free_space():
    # GN -1 nullifies the ground parameters and sets the free-space
    # condition (#1066); it must not read as "a GN card, so ground".
    bare = parse_nec("GW 1 3 0 0 1  1 0 1  0.001\nGE\nGN -1\nEX 0 1 2 0 1 0\nEN\n")
    assert bare.ground is False
    assert bare.ignored == ()
    assert bare.skipped_note() is None

    # A real GN followed by GN -1: the nullification wins, and the earlier
    # card no longer shows up as "not applied" because nothing survived.
    nulled = parse_nec(
    ...
    assert nulled.ground is False
    assert nulled.ignored == ()

    # The other way round, the later real GN stands.
    restored = parse_nec(
        "GW 1 3 0 0 1  1 0 1  0.001\n"
        "GE 1\n"
        "GN -1\n"
    ...
    assert restored.ground is True
    assert restored.ignored == ("GN",)


def test_skipped_note_ground_flag_only_and_clean_deck():
    # GE 1 alone (no GN card): ground is requested and nothing is in
    # `ignored`. The ground is seeded from the deck (AK#1432), so there is
    # nothing the app overrides and no note.
    flag_only = parse_nec("GW 1 3 0 0 1  1 0 1  0.001\nGE 1\nEX 0 1 2 0 1 0\nEN\n")
    assert flag_only.ignored == ()
    assert flag_only.skipped_note() is None
...
def test_errors_are_specific():
    ...
    # feature, not choke on the filename as an SY expression (#1067).
    with pytest.raises(ValueError, match="NL triangle-mesh surface"):
        parse_nec(
            "CE\nCM Generated by stl2nec5 (NL triangle mesh)\n"
            "NL hairpin.nl\nGW 1 1 0 0 -0.0016 0 0 0 0.001\nGE\nEN\n"
        )
