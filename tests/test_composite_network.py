...
def test_ft240_preset_pins_efhw_stock_values():
    ...
    # its own because the coated-wire model lowered its feed R ~19 % and no
    # ratio or length restores the match at 100 pF (best 1.327 against a 1.3
    # gate). 39 pF rather than the earlier 60: it is an E12 part and it sits
    # at the bottom of the SWR bowl, where a +-10 % part moves the rig 0.004.
    # Everything else — turns, magnetizing inductance, core Q — must still
    # come from the preset rather than drifting on its own.
    assert e.comp_c_pF == 47.0
