...
def build_engine(builder, engine, ground):
    ...
    if engine in ("bs1", "bs2", "bs3"):
        # The degree IS the basis here, which is why `density.py` gives bspline a
        # per-degree table and nothing else one.
        return MomwireEngine(
            builder,
            solver=BSplineSolver,
            solver_kwargs={"degree": int(engine[-1])},
            ground=ground,
        )
    if engine == "razor":
        return MomwireEngine(
...
def main(argv=None):
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="ENGINE:RUNG",
        help="run each engine at its OWN rung, e.g. bs2:15 bs1:20 bs3:12. Each "
        "engine here is served at one density, so there is no ladder and no "
        "convergence class -- only admissibility against the stored reference.",
    )
    ...
    if a.plan:
        ...
        for d in designs:
            for g in a.grounds:
                cells.extend((d, g, rung, e) for e, rung in plan)
    else:
        for d in designs:
