...
SERIES = {
    ...
    "bs2": ("#2a78d6", "momwire bs2 (corrected geometry)", "solid"),
    "bs1": ("#eb6834", "momwire bs1 (corrected geometry)", "solid"),
...
ax.plot(
    ...
    ls="none",
    zorder=6,
    label="electrostatic referee (858.7 pF)",
)
...
ax.set_title(
    "The coupled-loop ladder on the Smith chart (Z₀ = 371 kΩ) — CORRECTED (#518 postmortem)\n"
    'the original figure\'s "two clean limits 1.9 % apart" was a ladder-script geometry bug\n'
    "(wire 6 absent from the bs models); with it fixed, NEC-5, razor, bs1, bs2 and the\n"
    "electrostatic referee agree — only the NEC-2 scheme still walks the chart",
    color=INK,
    fontsize=10.5,
    pad=14,
)
fig.tight_layout()
fig.savefig(HERE / "coupled-loop-smith-corrected.png", dpi=160, facecolor=SURFACE)
