...
Run: .venv/bin/python scratch/qrz-lfa-thread/pathology_dissect.py
"""
...
# ---- n-gon ladder: corner angle at fixed perimeter/segments/current -------
for n_sides, segs in ((4, 32), (8, 32), (16, 32)):
    wires, npe, J = ngon_wires(n_sides, segs_total=segs)
    ...
    n_knots = sum(len(e) * 1 for e in npe) + len(npe)  # per-wire segs+1
    target = np.ones(sum(e[0] + 1 for e in npe), dtype=np.complex128)
    out, fit = circ_residual(mk, target)
    report(f"{n_sides:2d}-gon, {segs} segs, uniform 1 A", out, fit)

# ---- n_qp_const sensitivity on the square ---------------------------------
wires, npe, J = ngon_wires(4, segs_total=32)
for nqp in (8, 32):

    def mk(ch, wires=wires, npe=npe, J=J, nqp=nqp):
        return ChannelMasked(
            channels=ch,
            wires=wires,
            n_per_edge_per_wire=npe,
    ...
    target = np.ones(sum(e[0] + 1 for e in npe), dtype=np.complex128)
    out, fit = circ_residual(mk, target)
    report(f"square, n_qp_const={nqp}", out, fit)

# ---- Roy's model (k=1): channel split of the clean-current residual -------
from momwire import HarringtonSolver  # noqa: E402
...
loop_rows = np.concatenate([np.arange(firsts[w], lasts[w] + 1) for w in range(1, 6)])
