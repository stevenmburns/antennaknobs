...
`razor-2p` serves both above ground and, over Sommerfeld, below it. NEC-5's Michalski
...
| Buried wires / below-ground conductors | **Served** on the momwire `bspline` lane over the Sommerfeld ground — and, for every class below, on the `sinusoidal-galerkin` lane since momwire 0.52.0, a second basis sharing no fill code below the interface, and on `razor-2p` since momwire 0.62.0: impedance, currents, charges and (since antennaknobs v0.80.0) the radiation pattern for wires strictly below the interface (buried radials and screens, buried fed elements, elevated feeds over buried counterpoises), and an above-ground wire joined at the surface to buried wires at a declared junction (the connected radial screen). **Refused by name:** a wire crossing the interface mid-span; ground-contact wires mixed with buried wires; a wire lying *in* the interface plane (the surface class — radials lying *on* the ground — is served instead as the elevated family at an explicit small height, momwire#872; see below); and near fields of buried decks. A buried structure whose opposite tips are more than 4 in-medium wavelengths apart is served since momwire 0.55.0: past that range the below/below remainder is set to zero, a bound checked in impedance out to about 8 in-medium wavelengths on the buried-radial vertical's screen, and thinnest (a factor of 2) over near-lossless fresh water at 28 MHz. A licensed local NEC-5, asked with its documented below-ground card, agrees with momwire on the buried-radial vertical's connected deck to a fraction of an ohm in resistance (78.1+46.3j against 77.8+44.5j Ω at the design's defaults: 0.3 Ω in R at the shipped mesh, 0.2 Ω at twice it), on the wholly buried dipole to 0.2 % in resistance across depths, and on the elevated-detached class, which both serve as the same problem, at the 0.2–2 Ω level converged ([the buried-radials study](/advanced/buried-radials/)). The one buried spelling NEC-5 has no documented answer for is a conductor stopping on the interface above buried wires (the `detached` variant), which momwire refuses by name on its own physics. The PyNEC lane is gated off buried decks: nec2++ solves a buried wire as though it were in air and returns a plausible number, so the app withholds it with a sentence rather than serve it. The validation stance below ground is its own paragraph under this table. |
...
**Below the interface, validation stands on three legs.** The engine-independent
...
with each other on the depth ladder and on the with/without-radials coupling
differential. A buried number that survives a change of basis is a different
kind of evidence from one that survives a change of mesh, and both are now
asked. The crossing junction — the connected screen's node — joined that leg
last: on the shipped buried-radial vertical the two bases agree to within
their own discretisation error, and each sits the same distance from NEC-5,
so the residue is the node model and not either basis (momwire#980).
...
so the ordering and the crossing are reproduced and the benefit is under-stated.
...
with NEC-5 closer than the few-percent class stated above. **Refused by
name:** a wire lying *in* the interface plane. A buried structure whose opposite tips are
more than 4 in-medium wavelengths apart is served since momwire 0.55.0, with the
below/below remainder past that range set to zero; that bound was checked in
impedance out to about 8 in-medium wavelengths, and is thinnest (a factor of 2)
over near-lossless fresh water at 28 MHz.
