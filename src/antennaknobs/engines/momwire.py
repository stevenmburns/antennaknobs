...
# --- the faceted-terrain far-field composers ------------------------------
#
# `terrain_utd_power` is module-level rather than a method because it uses no
# engine state at all — only the moment set, the direction grid and the
# terrain — and because it now has a SECOND caller: the web layer's
# arbitrary-direction cuts path (`web/server.py::_mag2_at_directions`). That
# path used to mirror the specular composer in a second implementation; the
# diffracted field is not something to mirror twice, so it calls this.
#
# Callable on any slice of a grid: measured bit-identical column by column
# against the whole grid on the levee and the 45-degree hillside, which is
# what lets a caller holding an unstructured direction set group it by azimuth
# and evaluate each group here with the azimuth it belongs to.


def terrain_utd_power(
    terrain,
    sec_idx,
    mid,
    dr,
    ...
    * every break between facets adds the segment's UTD-diffracted field:
    ...
    pairs = sorted({(int(a), int(b)) for a, b in zip(sec_idx, back_idx, strict=True)})
    ...
    for fi, bi in pairs:
        ...
        for c0 in range(0, all_cols.size, chunk):
            ...
            for r in refls:
                ...
                E = np.exp(1j * k * np.einsum("tcx,cnx->tcn", rh, mid_i)) * _tcn(r.mask)
                comp_h = np.einsum("cnx,tcx->tcn", w_i, hh)  # h-component per segment
            ...
            for d in diffs:
                ...
                gate = _tcn(d.mask)
                S_s = np.sum(_tcn(d.D_soft) * gate * E_h[None, :, :], axis=-1) * ph_out
