...
<Card title="New in v0.79.0" icon="star">
  **The same wire on every engine.** An insulated wire is now modelled the same
  way whichever engine answers. NEC-2, NEC-5 and every deck this app exports
  carry the jacket the way momwire always has: as the larger radius the solver's
  kernel sees, plus the inductance that enlarging it would otherwise remove,
  with the conductor's own loss preserved. On the catalog's PVC-jacketed
  designs that closes a 10–14 % disagreement between engines to about a tenth
  of a percent, so their impedances move. The surface radial screen's mast and
  feed gap are bare, as the design always said they were — they had been
  inheriting the radials' jacket, worth about 26 Ω of reactance on the stock
  screen, so that design's numbers move too. On the solver side, a buried array
  with more than one element standing over its own ground connection now solves
  instead of being refused, buried screens that mesh finer than a twentieth of a
  degree fill faster, and an EZNEC drop-in printout names on its second line the
  version, basis and processor support that answered it.
</Card>
