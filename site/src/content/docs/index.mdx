...
<Card title="New in v0.55.0" icon="star">
  **momwire speaks the NEC-5 dialect.** momwire 0.35.0 adds a second
  portal alongside its NEC-2 one: a deck front end and byte-gated
  printout renderer for the NEC-5 card dialect — node addressing,
  all four ground cards, `TL`/`NT` networks, phased multi-`EX` drive —
  measured statement by statement against captured reference printouts,
  and refusing by name what it cannot yet stand behind. This release
  adopts the pin; the integration story comes later.
</Card>
