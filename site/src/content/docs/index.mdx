...
<Card title="New in v0.82.0" icon="star">
  **The decks EZNEC actually writes.** EZNEC's virtual wire — the short,
  distant wire its transformer and lossy-line idiom puts a source on — now
  imports as virtual circuit nodes instead of a refusal, so a model fed
  through a transformer and a lossy line opens on the command line, in the
  workbench, on momwire and on the NEC-5 engine. A deck EZNEC stamps
  "in NEC-5 format" is read as NEC-5: its sources, load probes, loads and
  network ends land on the knot the writer meant rather than half a segment
  away, while the NEC-2 and NEC-4.2 exports keep their NEC-2 reading — on a
  reported deck the NEC-5 engine now reproduces EZNEC's own printout to
  0.02 %. The SimNEC round trip is a fixed point: a re-imported `.ssn` keeps
  its segment counts (the export's one-time re-mesh at the feed is deliberate
  and stays). momwire 0.60.0 underneath: a one-field `GE` card — the shape
  AutoEZ's writer and EZNEC's NEC-2 Save As emit — is accepted instead of
  refused, and a two-drive model with load probes on both drives is gated
  against NEC-5.
</Card>
