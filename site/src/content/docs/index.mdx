...
<Card title="New in v0.52.0" icon="star">
  **The memory release.** momwire 0.29.0 lands its memory campaign:
  grounded and Sommerfeld solves now cost what free-space solves cost —
  certified at 8,320 basis functions, peak RSS 1.7–2.3 GB across every
  ground model (the Sommerfeld path alone came down from 11.7 GB), which
  moves the practical ceiling on a 16 GB machine from roughly 10k to
  15–18k basis functions. Long single-wire designs (the whip class) lose
  their biggest hidden transient, and answers are unchanged to printed
  precision throughout. Also new: the broadband **hexbeam** joins the
  [validation story](/reference/validation/) with two community-driven
  verdicts — the single-band catalog design (all three engines agree to
  0.15 Ω, the collection's first agreement entry) and the 5-band stack,
  including the physical one-coax feed solved as a network (two
  formulations within 1.2 Ω on every band). The momwire primer now lives
  at [momwire.dev](https://momwire.dev).
</Card>
