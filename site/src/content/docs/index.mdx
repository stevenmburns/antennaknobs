...
<Card title="New in v0.84.1" icon="star">
  **The circuit around the antenna, read as written.** A current source spelled
  NEC-2's way on a one-segment phantom wire now reads as the antenna rather
  than its reciprocal, and on the NEC-2 engine a design with a current source
  reports its feed rather than the inverse of it. SimNEC circuits import the
  way SimNEC writes them: component values with its SI suffixes (`731.9n`,
  `37.52p`, `2K`) and wire materials by name (`Conductivities.aluminum`). In the
  workbench, the mesh-density knob now says what it is, segments per quarter
  wavelength, and says so when a design loaded from a file keeps its own
  segments.
</Card>
