...
<Card title="New in v0.76.0" icon="star">
  **The two ohms.** A vertical bonded to buried radials, where the wire
  crosses from air into soil, now matches a licensed NEC-5 to a fraction of
  an ohm: 0.3 Ω in resistance on `verticals.buried_radial_vertical` at its
  shipped mesh, and 0.2 Ω at twice that mesh. The fix is in momwire 0.54.0's
  crossing kernel. It was derived from the field equations, not fitted, and
  it changes only designs with a conductor that crosses the ground plane.
  Every other buried-wire case in the catalog is unchanged. The
  [validation page](/reference/validation/) has the updated numbers,
  including the 1937 Brown–Lewis–Epstein radial-count comparison.
</Card>
