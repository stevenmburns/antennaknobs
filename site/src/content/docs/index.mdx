...
<Card title="New in v0.64.0" icon="star">
  **The NEC-5 twin stopped being expensive.** momwire 0.44.0 rebuilt the
  `razor` fill as a fused, threaded kernel, and the lane that was reserved
  for certification runs is now merely slow rather than prohibitive: a
  1600-segment free-space solve takes about 20 s inside 520 MB, where the
  CLI reference used to warn of an 8 GB working set — with every printed
  byte of the validation corpus unchanged. Same answers, a fraction of the
  machine. Nothing you write changes; the
  [CLI guidance](/reference/cli/) carries the new numbers.

  Under the hood this release is momwire 0.44.0 throughout, whose other
  half is the [EZNEC drop-in](https://momwire.dev/reference/eznec-nec5/):
  it now keeps a warm engine resident between EZNEC's per-point launches —
  **3.7× faster than the licensed NEC-5 console engine** on a 51-point
  sweep measured inside EZNEC itself — and ships as the first
