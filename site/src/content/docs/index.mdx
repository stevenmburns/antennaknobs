...
<Card title="New in v0.56.0" icon="star">
  **The NEC-5 portal answers near fields over every ground.** momwire
  0.37.0 ships a Sommerfeld point evaluator, so `NE`/`NH` requests serve
  over real ground and the MININEC-type ground with measured envelopes —
  119 of the 122 captured EZNEC launches in the reference corpus now
  come back with an answer, and the three that refuse share one honest
  sentence about one singular observation point. The corpus itself grew
  from 49 to 122 public captures since the last release; the
  [scored matrix](https://github.com/stevenmburns/antennaknobs/blob/main/docs/status/2026-08-20-eznec-nec5-scored-matrix.md)
  carries the re-score.
</Card>
