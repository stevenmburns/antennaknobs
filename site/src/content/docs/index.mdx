...
<Card title="New in v0.59.0" icon="star">
  **One name for the converged Galerkin solver.** The
  `sinusoidal-galerkin-converged` engine name is gone: the zero-width feed
  it selected is now the solver's own default, so plain
  `sinusoidal-galerkin` means what the suffixed name used to. On near-open
  high-Q feeds — the `wire.lazy_h` and `wire.vbeam` class — that removes two
  to three orders of magnitude of apparent disagreement between solver
  bases, and you no longer have to know to ask for it. **If you have a
  script or a saved slot spelling `-converged`, drop the suffix**; NEC's
  segment-wide gap survives as a feed-model control in the web panel for
  when reproducing NEC's mesh walk is the point. Details in the
  [CLI reference](/reference/cli/#choosing-an-engine).
</Card>
