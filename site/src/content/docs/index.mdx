...
<Card title="New in v0.69.0" icon="star">
  **Buried radial screens solve 4× faster, and the radial cap is gone.** A
  12-radial screen went from 11 s to under 3 s, the served buried deck from
  2.4 s to 1.1 s, and a 48-radial screen now solves where it used to refuse
  outright — under 20 s on a laptop, level with a licensed NEC-5's wall on
  the same deck, and about 27 s on the hosted app's smaller VM. Same
  impedance to every printed digit. Underneath:
  the off-edge pair quadrature follows each pair's distance
  ([momwire#906](https://github.com/stevenmburns/momwire/issues/906)), the
  buried fill streams through complex-medium C++ assemblers instead of a
  dense tensor ([momwire#915](https://github.com/stevenmburns/momwire/issues/915)),
  and the field-form Galerkin assembly is one fused C++ kernel
  ([momwire#914](https://github.com/stevenmburns/momwire/issues/914)).

  **Choose your soil.** The ground panel gains permittivity and
  conductivity knobs with named presets — very poor through salt water —
  carried through sweeps, patterns, cuts, near fields and the exported NEC
  deck ([#1173](https://github.com/stevenmburns/antennaknobs/issues/1173)).
  The default soil is unchanged, so nothing you saved moves.

  **No more mid-sweep stalls over lossy soil.** momwire keeps two
  Sommerfeld rungs and blends between them instead of refilling at every
  step, so a 20 m sweep over 20 / 0.03 earth stops pausing in the middle
  ([momwire#902](https://github.com/stevenmburns/momwire/issues/902)).

  **Free-space fills take the same shortcut.** The distance-adaptive
  quadrature now serves the real-wavenumber fill too — 1.5–2× on bent and
  multi-wire decks at one thread, with every pinned answer unmoved
