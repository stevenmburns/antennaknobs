...
<Card title="New in v0.53.0" icon="star">
  **The portal moves home.** The SimNEC engine portal now lives in
  momwire itself: `pip install momwire` provides the `momwire-nec2c`
  drop-in engine, serving momwire's own antenna-only NEC dialect with a
  [published grammar](https://momwire.dev/reference/deck-grammar-nec2/)
  and [usage notes](https://momwire.dev/reference/portal-usage/) — and
  its version probe now reports the engine's own version. momwire 0.31.0
  also completes the memory campaign's follow-through: the sinusoidal
  family joins the certified memory model, and large Sommerfeld solves
  come out *faster* than before the campaign. The 5-band **hexbeam** is
  now [tuned against its physical one-coax feed](/reference/validation/)
  — worst SWR 1.04 across all five bands, two independent formulations
  in consensus — and the NEC-5 reference page gains the full
  formulation-and-cost story behind the
  [validation panel](/reference/nec5/).
</Card>
