...
<Card title="New in v0.73.0" icon="star">
  **Download and double-click.** Every release now ships a signed Windows
  workbench: unzip, run `antennaknobs-workbench.exe`, and the server opens in
  your browser with momwire's C++ solver inside — no Python, nothing
  installed, delete the folder to remove it. A licensed NEC-5 joins with one
  text file beside the exe. **Patterns respect the ground.** A far field is
  read out only from the currents above the interface; a design whose
  radiating current is wholly buried refuses by name, and a mixed design says
  what share of its radiated power the imaged buried currents carry. **The
  app serves the one-wire crossing spelling**: a straight wire through the
  ground plane is split at the plane before the engine sees it. **A wrong
  design key is refused, never answered with another design's numbers**, and
  every multi-feed design solves on the NEC-5 tab. The README's install
  scripts are now what CI runs on every release, on three operating systems.
</Card>
