...
<Card title="New in v0.85.0" icon="star">
  **Tuners that tune themselves.** Give `l_network_tuner` or `t_network_tuner`
  a `tune_to` target instead of component values, and the engine solving the
  design picks the parts for its own antenna, then holds them across a sweep so
  you see the tuned bandwidth. A T keeps one part fixed (yours, or a capacitor
  parked at its maximum), and both take the component ranges a real box has.
  SimNEC's automatic LC match imports as this tuner and exports back as one.
  EZNEC's MININEC-type ground joins the finite grounds on every engine. The
  pattern views now say where the maximum is, with one click to find the whole
  pattern's peak and aim both cuts through it, and their captions stay clear of
  the controls on a small or zoomed display.
</Card>
