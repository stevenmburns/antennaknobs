...
<Card title="New in v0.64.0" icon="star">
  **The NEC-5 twin got its first C++ kernel.** `razor-nec5` is what you
  reach for when you want what NEC-5 would have said — and until now its
  matrix fill was plain NumPy, single-threaded, with no accelerated path
  at all. momwire 0.44.0 gives it a fused, tiled, threaded kernel, with
  every printed byte of the validation corpus unchanged. Measured on one
  box: a 1600-segment free-space solve in **1.1 s**, an 800-segment solve
  over real ground in **0.64 s**, both inside a few hundred MB, where the
  [CLI reference](/reference/cli/) used to warn about multi-gigabyte
  working sets at fine mesh. Same answers, a fraction of the machine, and
  nothing you write changes.

  The other half of momwire 0.44.0 is the
  [EZNEC drop-in](https://momwire.dev/reference/eznec-nec5/): it now keeps
  a warm engine resident between EZNEC's per-point launches — **3.7×
  faster than the licensed NEC-5 console engine** on a 51-point sweep
  measured inside EZNEC itself — and ships as the first
