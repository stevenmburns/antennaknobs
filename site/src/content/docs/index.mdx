...
<Card title="New in v0.88.0" icon="star">
  **Radial screens, converged and fast.** momwire 0.64 fixes a 1 % error on
  near-ground radial screens (the Sommerfeld remainder had not converged) and
  makes them several times faster: a 48-radial screen solves in about 4 s
  instead of 12.6 s, in a quarter of the memory, and 4nec2's GndScreen takes
  2.7 s. The Beverage's first razor-2p solve takes 8 s instead of 33 s.
  The workbench picks up a batch of AC6LA's requests: SimNEC `prm` and plain
  constants are knobs, a sweep saved suspended as `(y)` imports, the
  optimizer runs on the slot's own engine, and each design has its own Zo.
  The VSWR and S11 charts get a clickable axis range, an Auto that fits the
  dip, and a 2:1 line with a bandwidth readout. There is a combined Az + El
  pattern view, 3-D designs open in Iso, the **View** setting fixes the
  opening view, and the **?** in the header opens the docs.
</Card>
