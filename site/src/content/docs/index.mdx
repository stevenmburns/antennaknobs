...
<Card title="New in v0.83.0" icon="star">
  **The drive the deck meant, and where it sits.** A source at a wire end
  standing in the ground plane is a feed rather than a refusal, and it is now
  fed AT the contact instead of at the centre of the segment it stands in — on
  a base-fed vertical over ground those are two different drives, 29 % apart
  in X. A gyrator-emulated current source is read as the current source it is,
  not as a source behind a component, so a deck EZNEC wrote in its NEC-2
  dialect reports the antenna's impedance instead of its reciprocal; NEC-2
  export can now write that current source back out. EZNEC's own
  instrumentation idiom — a 1e-10 V probe dropped beside a load at a wire
  join — imports instead of colliding, and an `LD 2` jacket pair is read the
  way the card means it. momwire 0.61.0 underneath: a bonded ground rod on an
  ordinary antenna is served, and above the interface the wires may carry
  several different radii.
</Card>
