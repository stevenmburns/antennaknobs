...
<Card title="New in v0.70.0" icon="star">
  **Hold the target while you drag.** Mark a knob to optimize, choose
  Resonance or Match Z₀, switch on *Keep the target while I drag*, and the
  marked knob follows every other knob you move so the antenna stays resonant
  (or matched) under your hand — one solve per drag tick. When the target
  genuinely cannot be held any more the knob freezes at its last good value
  and the VFO row says so; it re-acquires as soon as it can. **The optimizer
  treats a match as a root, not a minimum**: one knob to X = 0 in 3 to 5
  solves, two knobs to Z₀ in 11 to 13, where Nelder–Mead took 10 to 48 and
  sometimes never converged. **Solvers now tell you what they are unsure
  about**: a new advisories panel surfaces the warnings momwire raises during a
  solve instead of discarding them, buried decks are refused by name on the
  backends that would solve them as though in air, and the soil advisory flags
  where the single-valued (εr, σ) model stops being trustworthy. Under it,
  momwire v0.50.0 solves a 48-radial screen in **14.9 s and 1.0 GB**, from
  16.6 s and 1.5 GB, at the same impedance to nine digits.
</Card>
