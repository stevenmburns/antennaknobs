...
<Card title="New in v0.57.0" icon="star">
  **Wires below the interface.** momwire 0.38.0 lands the buried serve:
  buried radials, screens, and buried fed elements get impedance,
  currents, and charges over the Sommerfeld ground on the `bspline`
  lane — at deck level today — and the edges are named rather than
  papered over: interface crossings, ground-contact wires mixed with
  buried wires, and near fields or patterns of buried decks refuse by
  name. Validation below ground is deliberately engine-independent; the
  [solver guide](/reference/solver/#honest-limitations) and the
  [validation page](/reference/validation/#honest-limits) carry the
  story.
</Card>
