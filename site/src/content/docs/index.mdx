...
<Card title="New in v0.67.0" icon="star">
  **Underground, the reference is now a measurement.** Brown, Lewis and
  Epstein's 1937 buried-radial screens — base resistance against radial
  count, measured with a meter at 3 MHz — are gated in momwire 0.47.0
  ([#838](https://github.com/stevenmburns/momwire/issues/838)) for both the
  45 ft and 135 ft radial lengths. It is the first below-ground reference in
  either project that was not another program's output.

  **Two readings of one engine.** Below ground the engine is the B-spline
  basis at degree 2, and its second reading is the same solver at degree 1:
  the pair agrees within about 1 Ω on every buried anchor and on the
  catalog's buried radial vertical, and the
  [validation page](/reference/validation/) now says so, with the table.
  That replaces the NEC-5 comparison underground — deliberately. NEC-5's
  interface node was measured against the same 1937 curves this week and
  comes out nearly flat in radial count; we built the NEC-5-style crossing
  fill, measured it, and chose not to ship it as the buried engine
  ([#813](https://github.com/stevenmburns/momwire/issues/813)). `razor-2p`
  stays what it is: the above-ground twin of licensed NEC-5.

  **It now tells you its accuracy class.** Building a `razor-2p` solver
  states, once, how far its path-tested rows sit from converged at the
  catalog mesh — median 3.3 %, tail 34 %, halving per mesh doubling
  ([#845](https://github.com/stevenmburns/momwire/issues/845)). The number
  comes from a committed sweep of 88 decks, not from prose.

  **Buried decks solve at converged quadrature out of the box.** The
  cross-edge order now follows the geometry: 32 when any wire sits below the
  interface, 8 otherwise, and the hosted simulator's control defaults to
  *auto* so that decision reaches you
  ([#760](https://github.com/stevenmburns/momwire/issues/760)). On the
  graded four-radial screen that moves the answer 2.5 Ω toward its
  converged value for a 1–2 % cost; on decks standing clear of the ground it
  changes nothing.
</Card>
