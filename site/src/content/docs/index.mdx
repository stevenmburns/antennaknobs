...
<Card title="New in v0.62.0" icon="star">
  **Buried solves got ~4× faster.** momwire 0.42.0 rebuilt the
  mixed-medium crossing fill around an admissibility split: pairs near
  the ground interface keep the designed dense evaluation, far pairs run
  on coarser quadrature (with a low-rank pass on dense meshes), and a
  deep-deck ladder then proved the near pairs could carry the same
  density too. Together with a screen-symmetry fix in the catalog
  geometry, the buried-radial vertical's warm solve went from ~12 s to
  **~3 s** — with every certification anchor reproduced to a few 10⁻⁴ Ω.
  The K&gt;2 junction node (a monopole meeting several buried radials)
  also gained a **convergence study**: the composition's residual is now
  a measured first-order class with a graded-mesh recipe that converges
  it to the measurement floor, and the four-radial fan carries a banked,
  converged soil-A anchor for the first time. Faster answers, same
  physics, better-pinned.
</Card>
