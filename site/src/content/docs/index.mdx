...
<Card title="New in v0.51.0" icon="star">
  **The apex feed.** Feed an inverted vee at its **exact vertex** — no
  bridge wire — the way NEC-5 users build them natively. momwire 0.28
  adds the series node gap (a voltage in series with a named wire-end at
  a junction, distinct from the shunt junction port), and the new
  `PortAtVertex` port drives it on **every momwire family, over every
  ground model**, and on NEC-5 through its native `EX`-at-the-knot
  source; NEC-2-shaped engines refuse honestly rather than substitute.
  The new [`dipoles.invvee_apex`](/reference/catalog/) design makes the
  old question a number: the bridge-feed idiom is worth **about 2 Ω,
  nearly all reactance**, on a stock 10 m vee — and the NEC-5
  cross-check lands **0.11 Ω** from momwire under the validation
  pair recipe. Also new: the
  [validation story](/reference/validation/) pages with the Leeson
  tapered-dipole demo, the SimNEC portal serving the `IS`
  insulated-sheath card, and standalone portal runs — a stock `.nec`
  deck ending in `EN` now solves directly from the shell.
</Card>
