...
<Card title="New in v0.87.1" icon="star">
  **The file's own knobs.** A 4nec2 deck written with `SY` cards and a SimNEC
  file written with `dcl` constants now import with those constants as
  knobs: each is labelled with its name as the file spells it, its comment is
  the tooltip, and the values the file derives from them follow as you drag.
  In v0.87.1 a SimNEC element's own parameters (a bare `len;` whose value
  SimNEC saves with the circuit) are knobs too, including one that sets a
  wire's `JamSegments` count.
  SimNEC's expressions are read by SimNEC's own rules (trig in radians, SI
  suffixes, case-sensitive names), measured against SimNEC itself. The
  catalog gains a Beverage receiving antenna with ground rods at both ends
  and a receiving-directivity (RDF) readout, the cancel button now stops
  every server-side computation, and momwire 0.63 makes near-ground and
  buried screens much faster: one impedance solve of 4nec2's GndScreen takes
  about 10 s instead of 276 s.
</Card>
