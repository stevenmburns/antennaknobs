...
<Card title="New in v0.50.0" icon="star">
  **The Galerkin kernel unlocked.** momwire 0.27.0 finishes NEC's extended
  thin-wire kernel on the **Sin-Galerkin basis** — every ground model
  including full Sommerfeld, bent and stepped geometry made sound by a
  node-gated end-bracket correction — so **all five momwire solver
  families now serve the `EK` card**, in the workbench gear menu, on the
  CLI, and for SimNEC decks through the portal (which sends `EK` on every
  live solve). Ground-contact decks over lossy soil now **converge under
  the extended kernel** instead of walking, and swept solves on the
  accelerated solvers **dispatch to the fastest engine automatically**.
  The cross-validation triangle also closed its one open gap: the
  low-height reactance split published in v0.49 was **resolved by the
  full corpus study** — a feed-model convergence term, not physics, gone
  under Richardson extrapolation — and the 3,146-deck wild-corpus census
  put the triangle to work, tracing 46 formulation outliers to NEC-2's
  known stepped-diameter defect. Start at
  [the extended kernel](/reference/solver/#the-extended-thin-wire-kernel-ek).
</Card>
