...
<Card title="New in v0.76.1" icon="star">
  **The deck as you wrote it.** A NEC-5 deck now opens as written: its
  `GN … NOFILE` card is understood, its `GN 0` is read as the Sommerfeld
  ground NEC-5 means by it, and a source on a wire's middle knot keeps that
  wire whole instead of cutting it in two. Feed markers sit on the source on
  every engine tab. The packaged workbench takes the engine path on its
  command line as `--nec5-exe` / `--nec2-exe`, which reads the same in
  PowerShell and Command Prompt. `wire.terminated_longwire` now solves over
  finite ground, and `antennaknobs ladder` runs an imported deck at refined
  meshes on every engine.
</Card>
