...
<Card title="New in v0.61.0" icon="star">
  **Both engines, both conventions.** The buried-radial vertical now drives
  through a licensed local NEC-5 too: the wrapper serves **buried decks**
  (NEC-5's native `GE 1,-1` + Sommerfeld support), and the design grows a
  `detached` variant — the **stake convention**, radials lying in the dirt
  touching nothing — which is the spelling NEC-5's own junction convention
  serves. momwire solves the default connected screen and refuses
  `detached`; NEC-5 solves `detached` and refuses the default's coincident
  bundle — each refusal names the variant the other engine serves, because
  the two conventions are **two different antennas**, tens of ohms apart.
  Also: `--engine nec5` finally shows itself in the CLI help (it was
  always there, gated on `$NEC5_EXE`), and the buried catalog designs now
  declare their ground — the web app auto-selects finite + Sommerfeld on
  load instead of walking you into a refusal. NEC-5 stays strictly local
  (pip users with their own licensed binary; the hosted simulator never
  ships it) — see [NEC-5 as a third engine](/reference/nec5/).
</Card>
