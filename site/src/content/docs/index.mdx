...
<Card title="New in v0.54.0" icon="star">
  **The razor basis arrives.** momwire 0.32.0's `RazorSolver` — the
  NEC-5 formulation twin (tent basis, razor-blade testing, the same
  method the reference code itself uses) — is now a selectable engine:
  `--engine momwire:razor-nec5` for interactive work, reproducing the
  licensed binary's printed impedances to a fixed few-hundredths-of-an-ohm
  offset, and `momwire:razor` as the converged reference lane. The row
  serves all three ground models clear of the plane, **base-fed monopoles
  and grounded wire ends over PEC**, wire loading, per-wire radii, and
  `.nec` decks — and what it refuses, it refuses with the reason in the
  message. See the [solver reference](/reference/solver/) for the
  capability table and the measured performance guidance.
</Card>
