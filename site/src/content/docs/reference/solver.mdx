...
joining the radials at depth is allowed). That crossing junction is served
on the `bspline` lane; the `sinusoidal-galerkin` lane refuses it by name
until it has a crossing serve of its own
([momwire#980](https://github.com/stevenmburns/momwire/issues/980)), and
the app greys the tab with that sentence before a solve is attempted. The [design catalog](/reference/catalog/) now authors buried
...
the served path), the crossing junction on the sinusoidal-Galerkin lane,
