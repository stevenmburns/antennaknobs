...
- **Junction-port (`PortAtEnd`) designs** accept `sinusoidal-galerkin`,
  ...
  sinusoidal and pulse bases refuse them, with momwire's own sentence.
...
`bspline` lane over the Sommerfeld ground, and since momwire 0.52.0 on the
`sinusoidal-galerkin` lane as well. The two bases share no fill code below
...
the served path), coincident rises meeting at a node on the sinusoidal-Galerkin lane,
and near fields or patterns of buried decks. Validation
