...
`bspline` lane over the Sommerfeld ground. The capability is deck-level
today: imported decks bring their own segmentation and solve directly, while
the design builder does not yet author buried geometry (in-medium
auto-meshing is
[antennaknobs#983](https://github.com/stevenmburns/antennaknobs/issues/983)).
What still refuses does so by name: a wire crossing the interface, a
ground-contact wire combined with buried wires, and near fields or patterns
of buried decks. Validation below ground is deliberately engine-independent —
...
below-interface Sommerfeld tables — a minute or two today (accelerated fills
are momwire#568).
