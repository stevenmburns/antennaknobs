...
Both are **opt-in, and stay that way**. Over a real ground the H-matrix
lane compresses the Sommerfeld part of the matrix with a global low-rank
factorisation, and on compact multi-wire decks that factorisation can stop
early without the stopping test noticing: across the catalog at the default
tolerance, 32 of 99 designs read more than 1e-6 from the dense answer and
the worst reads 3e-4
([census](https://github.com/stevenmburns/antennaknobs/blob/main/docs/status/2026-09-09-hmatrix-catalog-residual-census.md),
[momwire#1019](https://github.com/stevenmburns/momwire/issues/1019)). The
solver publishes its remainder residual as a diagnostic and falls back to
the dense fill when the miss is gross, but no threshold separates the good
decks from the bad, so no bar is applied: the accelerated lanes are for
large problems the dense solver cannot hold, and a number you intend to
quote comes from the dense `bspline` lane (decision 2026-09-10).
...
formulation-twin role needs, and refuses the rest with a named message
rather than a silent wrong answer:

- **No junction ports, and no feed or load at a junction of three or more
  ends** — a junction basis is already a through-current unknown, and the
  branch pair a source or load would drive is ambiguous at three or more
  coincident ends. A series node gap there names its arm, and is served.
  Ordinary K=2 junctions (a wire split, or two wires meeting end to end)
  are unaffected — those resolve to the same tent basis automatically, from
  the geometry, with nothing to configure.
- **No feed named at a segment centre** — razor's ports sit on knots, so a
  feed addressed the way the NEC-2 dialect addresses it would land half a
  cell from where it was named.
- **No singular enrichment.**
- **No ground contact under the reflection-coefficient ground** — that
  ground's model has no validity at zero clearance, and every momwire
  family refuses the combination.

Served: free space and all three grounds (PEC, reflection-coefficient,
Sommerfeld); the extended kernel; series node gaps (a `PortAtVertex` feed);
ground contact over PEC — including a base-fed monopole — and over the
Sommerfeld ground; buried and interface-crossing decks over the Sommerfeld
ground (see *Below the interface*, under
[Honest limitations](#honest-limitations)); per-wire radii; and both distributed and lumped wire loading. Full detail: momwire's
...
momwire serves it on six solver families — **`sinusoidal`**,
**`sinusoidal-galerkin`**, **`bspline`**, **`razor-2p`**, **`hmatrix`** and
...
bent elements, junctions) sound. What refuses it does so loudly rather
than silently:

- **the `pulse` basis** — reduced-kernel only.
- **singular enrichment**, on the bases that otherwise serve EK — the
  ...
  moment kernels the extended kernel corrects (momwire#249).
- **a wire below the ground plane**, on every lane — the extended kernel
  has not been measured on a pair of segments spanning two media. Solve a
  buried deck with the reduced kernel, which is the default.
- **a radius step at a junction** on `sinusoidal-galerkin` — measured
  divergent; `bspline` and `sinusoidal` serve the step.
...
- **Junction-port (`PortAtEnd`) designs** accept `sinusoidal-galerkin`,
  `hmatrix` and `arrayblock` beside the default `bspline` (the two
  accelerators were measured on the Sterba curtain in #1152 and agree with
  the dense parent to machine precision as their tolerances tighten; the
  sin-Galerkin junction ports run in free space and over a PEC ground —
  momwire#191 — with finite grounds still refused). The point-matched
  sinusoidal and pulse bases and `razor-2p` refuse them, with momwire's
  own sentence.

## Segments & convergence

Method-of-moments discretizes each wire into **segments**, and the solve builds
one **basis function** per segment. The **segments per λ/4 (N)** control in the
...
velocity factor. Since v0.79.0 every NEC deck this app writes carries the same
coated-wire pair momwire models rather than an approximation of it: the larger
equivalent radius the kernel sees, the jacket's own inductance as an `LD 2`
card, and the conductor's loss preserved by rescaling `LD 5` for that radius. Every momwire solver carries
...
ground **contact**, as distinct from grazing above it — is served over the
Sommerfeld ground on the `bspline`, `sinusoidal`, `sinusoidal-galerkin` and
`razor-2p` lanes
([momwire#624](https://github.com/stevenmburns/momwire/issues/624)), and
refused by name under the reflection-coefficient ground, whose model has no
validity at zero clearance. And the
...
buried counterpoises — serve **impedance, currents and charges**, and
since antennaknobs v0.80.0 the **radiation pattern**, on the `bspline` lane over the
Sommerfeld ground, and since momwire 0.52.0 on the `sinusoidal-galerkin`
lane as well. The two bases share no fill code below
the interface, so a buried number now has an in-house second opinion from a
different formulation rather than a re-run of the same one. Since momwire
0.62.0 `razor-2p` — the NEC-5 formulation twin — serves buried and crossing
decks too, and its buried fill is faster and leaner than the earlier lanes'.
Since momwire
0.41.0 a wire may also **cross the interface at a declared junction**: a
buried wire whose end stands in the ground plane, junction-joined there to
one above-ground wire (one above × N below, so a connected radial screen
spells as N rises meeting the monopole at the surface, and a buried hub
joining the radials at depth is allowed). That crossing junction is served on
both lanes since momwire 0.52.0: the sinusoidal-Galerkin node carries its own
degrees of freedom, and continuity across the interface emerges from the
fill rather than being imposed
([momwire#980](https://github.com/stevenmburns/momwire/issues/980)). Since
momwire 0.56.0 a deck may carry **several** such nodes, so a multi-element
buried array — two base-fed verticals each over their own screen, say — solves
instead of being refused on a one-node scope. The
one spelling that lane refuses by name is N coincident rises meeting at the
node — the retired `bundle` variant — which stays `bspline`-only; the app
greys the tab with momwire's sentence before a solve is attempted. The [design catalog](/reference/catalog/) now authors buried
...
the served path), coincident rises meeting at a node on the
sinusoidal-Galerkin and `razor-2p` lanes, and near fields of buried decks. Validation
...
(the [validation page](/reference/validation/) carries the numbers). The
