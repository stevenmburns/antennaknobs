...
- **`razor`** / **`razor-2p`** — a tent basis tested by NEC-5's own
  razor-blade rule rather than point matching or Galerkin testing — see
  below.

## Razor: the NEC-5 formulation twin

`razor` and `razor-2p` are one solver class (`RazorSolver`) offered as two
...
`razor-2p` additionally binds NEC-5's identified quadrature rule for the
...
**Reach for `razor-2p`.** Between the two lanes it is the one to use: it
binds the two-point rule momwire#316 identified for the testing-path
integral, where plain `razor` spends roughly 10× the wall time on
Gauss-Legendre nodes for an advantage that has evaporated by N≈192.
...
Plain `razor` came first — it was built as the twin, before momwire#316's
residue study identified the two-point quadrature rule that made the match
near-exact — and what it is now is an *instrument*, not a third choice: it
answers "is this coarse-mesh disagreement the testing rule or NEC-5's
quadrature shortcut?", and nothing a model needs. It costs about 20× the
time for that (free space at N=1600, one box, momwire 0.44.0: 20.2 s
...
- **`razor` / `razor-2p`** — reduced-kernel only, by design: NEC-5's
