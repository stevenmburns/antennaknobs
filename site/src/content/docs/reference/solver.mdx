...
**Reach for `razor-nec5`.** It is the twin: the one that answers what
NEC-5 would have said, which is the only reason to be on this solver at
all. If that is not the question, `bspline-d2` is the better answer.
...
against `razor-nec5`'s 0.97 s) and, since momwire#742 gave both lanes the
