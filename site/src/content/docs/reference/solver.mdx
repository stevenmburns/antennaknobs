...
- **a wire below the ground plane**, on every lane — the kernel's O(a²)
  tube expansion is written at one wavenumber, and a buried deck spans two
  media. Solve it with the reduced kernel.
...
since v0.80.0 the **radiation pattern**, on the `bspline` lane over the
