...
- **`ex=1 + 0j`** — this wire is the **driven** element; the source sits on it.
  ...
  `ex=` always feeds the **midpoint of the edge it sits on**, so putting it on a
  full-length radiator is a mid-element tap rather than a base feed: a
  base-fed vertical is a *short* driven wire spanning the ground plane with
  `ex=` on it, joined to the radiator.
