...
program needs `_internal` beside it). It is the same program as the pip
install below, packaged, with momwire's C++ solver inside.

[The Windows workbench](/start/workbench/) covers it properly — the options,
the self-test, and how to point it at a licensed NEC-5 engine (if you run
EZNEC Pro+ you already have one, in EZNEC's `Docs` folder rather than under
`Program Files`).
